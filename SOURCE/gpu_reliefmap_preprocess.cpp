...
void CALLBACK OnFrameRender( IDirect3DDevice9* d3dDevice, double Time, float ElapsedTime, void* UserContext )
{
	...
	if (g_Effect && g_ReliefTexture)
	{
		...
		if ( g_RunMode == RUNMODE_RUNNING )
		{
			...
			Offset.x = g_currentTexelIndex % g_ReliefSizeX * 1.0f / g_ReliefSizeX + 0.5f / g_ReliefSizeX;
			Offset.y = g_currentTexelIndex++ / g_ReliefSizeY * 1.0f / g_ReliefSizeY + 0.5f / g_ReliefSizeY;
			Offset.z = 0;

			if(g_currentTexelIndex == g_ReliefSizeX * g_ReliefSizeY)
			{
				g_RunMode=0;
				CDXUTButton *Button = (CDXUTButton *)g_DlgOptions.GetControl(IDC_START);
				Button->SetText("Start");
